...
| 배송(fulfillment) 웹훅 | ✅ (공유 시크릿) | `POST /api/v1/webhooks/fulfillment` (`X-Webhook-Secret` 필요) → `order_cache` 갱신 — `order/webhook.controller.ts` |
...
| OAuth(공개 앱 설치) | ✅ (키 필요) | `GET /api/v1/auth/shopify/{install,callback}` — 인가 리다이렉트·쿼리 HMAC·state·code교환·테넌트 upsert. `SHOPIFY_API_KEY/SECRET/APP_URL` 필요(§8) |
...
## 8. 경로 B — 공개 앱(OAuth) 확장 (✅ 구현됨 · 키 설정 필요)

다수 테넌트 SaaS용 OAuth 설치 플로우가 구현됐습니다. `SHOPIFY_API_KEY`/`SHOPIFY_API_SECRET`/`SHOPIFY_APP_URL`이 설정돼야 활성화되고, 미설정 시 엔드포인트는 501을 반환합니다.

### 8.1 Partner 대시보드 앱 설정
- 앱 생성 → **App URL**, **Allowed redirection URL(s)**: `<SHOPIFY_APP_URL>/api/v1/auth/shopify/callback`.
...
GET /api/v1/auth/shopify/install?shop=<shop>.myshopify.com   (@Public)
   → state nonce 발급(Redis, 10분) → Shopify 인가 URL로 302 리다이렉트
GET /api/v1/auth/shopify/callback?code&hmac&shop&state        (@Public)
   → 쿼리 HMAC(hex)·state nonce 검증(불일치 401)
   → POST /admin/oauth/access_token 으로 code→토큰 교환
   → tenant를 shop_domain으로 upsert
   → integration_credentials(provider=shopify)에 토큰 암호화 저장
   → 앱으로 리다이렉트(?shopify_connected=<shop>)
```
> 🟡 남은 로드맵: 설치 시 웹훅(GDPR+운영) 자동 등록, 선택적 ScriptTag 주입은 후속. (현재는 콘솔에서 §4/§5 웹훅 URL을 앱에 수동 등록.)

환경변수:
```bash
SHOPIFY_API_KEY=<Partner 앱 client id>
SHOPIFY_API_SECRET=<Partner 앱 client secret>
...
- [x] (경로 B) `/auth/shopify` OAuth(install·callback) — 적용됨(키 설정 필요). 설치 시 웹훅/ScriptTag 자동 등록은 로드맵
