...
1. ~~브라우저 실측 B-01~B-06~~ → **2026-08-13 완료, 6/6 통과** (TCR §3). B-07/B-08도 통과.
   ⚠️ 관측 O-1: **적용한 규칙의 효과가 재실행 답변에 드러나지 않았다.** 전환·재실행 동작(B-05)은
   통과했으나, 규칙이 실제로 답변을 바꾸는지는 이 방식으로 확인할 수 없다 — **W4 회귀 검증이 필요한
   이유가 실측으로 확인된 셈**이다.
