...
| 동기화가 **만든** 문서 | **0건** — 현재 지식 전량은 카탈로그 동기화·CSV·수기·갭 승격 |
| board 소스 1번에 **묶여 있는** 문서 | **12건** — 시드가 만든 tenant 1의 배송·반품·보증·CS 정책 문서 (`seed.runner.ts:178,201`이 `sourceId`를 붙임) |
...
- **권장: `board`를 소스 유형에서 내립니다**(`repository`와 동일 처리). 다만 **기존 소스 행은
  지우지 않습니다** — 시드가 새 테넌트마다 board 소스를 만들고 **기본 정책 문서 12건을 거기에
  묶어두기 때문입니다**(`seed.runner.ts`). 유형을 내리는 것과 행을 지우는 것은 다릅니다:
  행이 남아 있는 한 문서는 그대로 인용됩니다.
- 여기서 파생되는 두 가지:
  ① 시드는 앞으로 문서를 `source_id = NULL`로 씁니다. 손으로 쓴 지식에 통로를 붙일 이유가
     없고, 붙어 있으면 **소스 하나를 지정 해제하는 것만으로 테넌트의 기본 정책이 통째로
     검색에서 빠집니다**(PR #387 이후 실제로 그렇게 동작합니다).
  ② `NOT IN (지정 해제된 소스)` 형태 덕분에 **소스 행이 사라져도 문서는 남습니다.**
     `IN (지정된 소스)`로 썼다면 소스를 지운 순간 문서가 조용히 사라졌을 것입니다.
