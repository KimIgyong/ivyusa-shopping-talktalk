...
### 1.4 "해결됨" 상태는 없지만, 신호는 이미 남고 있습니다
`conversations`에 "해결됨" 상태는 없습니다(`status`는 ai_active/waiting/agent/ended).
그러나 **`IdleConversationService`가 이미 그 자취를 만듭니다** — 양쪽 침묵 30분
(`IDLE_PROMPT_AFTER_MIN`) 뒤 안내 문구를 보내고(`chat.idle_prompted`) 유예 후
종료합니다(`chat.idle_closed`). 운영 확정(2026-08-25)에 따라 **이 묻고-닫음을 해결 신호로**
씁니다.

⚠️ 다만 그대로 쓰면 안 됩니다. 묻고-닫음은 **답을 받고 조용해진 고객**과 **답을 못 받고 떠난
고객**을 똑같이 덮습니다. 구분 신호는 **침묵 직전 마지막 발화가 어느 쪽인가**입니다 → D3.

⚠️ 또한 `close()`가 `idle_prompt_at`을 **NULL로 지웁니다.** 종료된 행만 봐서는 물어봤는지
알 수 없으므로, 근거는 **감사 기록이나 남은 안내 메시지**에서 읽어야 합니다.
...
| **D3** | "해결 소요시간" 정의 | **확정(2026-08-25)**: 상담원 종료 · CSAT 응답 · **묻고-닫음** 을 해결로 집계하되, 묻고-닫음은 **침묵 직전 마지막 발화가 상담원/AI일 때만**. 마지막 발화가 고객이거나 말 없이 닫힌 방치 종료는 **건수만 별도 표기** | — |
...
가장 주의할 것 셋:
- **해결 신호는 이미 있지만 그대로 쓰면 안 됩니다.** 묻고-닫음은 답을 받은 고객과 떠난 고객을
  똑같이 덮으므로, **침묵 직전 마지막 발화**로 갈라야 합니다. 안 그러면 포기한 대화가 해결
  목록에 들어가고 그 비율은 서비스가 나빠질수록 올라갑니다(D3).
