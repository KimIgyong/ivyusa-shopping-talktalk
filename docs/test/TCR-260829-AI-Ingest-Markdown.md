...
| U5 | (기존) `video.mp4` | 여전히 **E5066** | PASS |
| U6 | (기존) 200,000자 초과 | `truncated: true`, 절단 | PASS |

`apps/api/src/domain/knowledge/knowledge-ingest.service.spec.ts`

| # | 케이스 | 기대 | 결과 |
|---|---|---|---|
| U7 | `manual.md` 업로드 | 확장자 게이트 통과 → 잡 READY, `sourceLabel = manual.md`, **모델 프롬프트에 `# 체크인 안내` 헤딩이 그대로 실림** | PASS |
| U8 | (기존) `x.hwp` | 요청 단계에서 E5066, 잡 생성 0 | PASS |
...
| 항목 | 결과 |
|---|---|
| `npm --workspace apps/api run test` | **178 suites / 1,791 tests 전부 PASS** |
