...
| main 머지 | ✅ PR #168 → `0dd84aa`, 후속 수정 PR #170 → `bce1cd2` |
| staging (`shoptalk.amoeba.site`) | ✅ **배포 완료 2026-08-08** (2회: 초회 `0dd84aa`, D5/D6 수정 후 `bce1cd2`). 부팅 로그 `Nest application successfully started`, 신규 라우트 401(=배포됨), health ok |
...
2. **카테고리명이 필요하면** `mall.read_category` 스코프를 추가해 재설치·재동의(현재는 403 → `category` 빈 값).
