...
> **정정(2026-08-26)**: 아래 ①에서 "소스는 아무 기여도 하지 않는다"고 쓴 것은 절반만
> 맞습니다. 동기화가 만든 문서는 0건이지만, **시드가 board 소스 1번에 정책 문서 12건을
> 묶어두었습니다**(`seed.runner.ts`). 동기화된 적이 없다는 것과 소스에 묶인 문서가 없다는
> 것은 다른 이야기이고, PR #387 이후에는 그 소스를 지정 해제하면 12건이 검색에서 빠집니다.
