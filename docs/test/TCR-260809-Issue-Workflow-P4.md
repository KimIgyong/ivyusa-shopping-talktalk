...
tenant 3(amoebaorder)의 master 계정으로 실행(계정 주소는 콘솔/DB에서 확인 — 문서에는 남기지 않는다).

⚠️ **로그인은 반드시 상점 슬러그 주소(`/<slug>`)로** 해야 한다. `shop_domain`으로는 테넌트가
안 잡히고, 이 계정과 **같은 이메일이 tenant 2에도 존재**해서 슬러그가 없으면 엉뚱한 테넌트로
조회돼 E1002가 난다.

⚠️ 이 함정은 2026-08-19에 실제로 잠금 사고를 냈다 — 다른 상점 주소에서 시도한 2건이
**성공 불가능한데도** 실패 카운터를 소모해 5회 잠금(E1008, 15분)에 걸렸다.
대응으로 로그인 화면에 상점 확인 힌트를 추가했다(`PLN-260819-Login-Wrong-Store-Hint`).
