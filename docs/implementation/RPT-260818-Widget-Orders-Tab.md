...
- 배포: local ✅ / **staging ✅ 2026-08-18** (`fc807d1` + 후속 `a6d81f7`) / production ❌
...
| # | 항목 | 비고 |
|---|---|---|
| ~~N-1~~ | ~~TCR S-1~S-14~~ | **S-1·S-3·S-5·S-8 통과(§8)**. 나머지는 미수행 |
...
## 8. 스테이징 배포 · 검증 (2026-08-18)

머지 `fc807d1`(#310) → 배포 → 육안 검증 → 결함 1건 발견 → `a6d81f7`(#311) → 재배포.
마이그레이션 없음(스키마 무변경).

| 항목 | 결과 |
|---|---|
| 배포 | 컨테이너 재생성, `Nest application successfully started`, api healthy, `/health` 200 |
| **S-1 판정 기준** | ✅ 고객 8 로그인 → 주문탭 → **`1004 / 결제완료 / The Collection Snowboard: Liquid / 2026년 8월 10일 · $749.95`**. 이전에는 어느 화면에서도 볼 수 없던 주문 |
| S-2 진입 칩 | ✅ `주문내역`이 선택된 채로 목록이 바로 보임 |
| S-3 배송중 단계 바 | ✅ 고객 4의 `1001 / 배송중`에서 행 안에 4단계 바(2단계 활성) |
| S-4 결제완료 | ✅ 뱃지만, 단계 바 없음 |
| S-5 상세 진입 | ✅ 품목·합계·`배송 조회`·`이 주문에 대해 문의하기` |
| S-8 배송 칩 회귀 | ✅ 기존 뷰 유지("진행 중인 배송이 없습니다") |
| 공통 "더보기" | ✅ 주문내역에서는 숨김, 배송 칩에서는 표시(리뷰 R-3 확인) |

### 8-1. 검증 중 발견한 결함 → FIX-260818

목록은 `결제완료`인데 **상세는 `Confirmed`**. 상태 라벨을 신규 목록에만 연결하고 기존 화면
둘을 빠뜨렸다. 고치면서 `OrderDetail`의 `/deliver|complete/`("Delivery failed"·"Incomplete"를
배송완료로 판정)와 `ShipmentList`의 `/ship|transit|deliver|fulfil/`(`Unfulfilled`)도 함께 정리.
→ `docs/bug-fix/FIX-260818-Order-Status-Label-Split.md`, PR #311.
재배포 후 상세에서 `#1004 결제완료` 확인.

### 8-2. 결함이 아니었던 것

배송 단계 라벨이 영어로 보였으나, 서버가 `session.language` 기준으로 이미 번역된 라벨을
보내고 해당 테스트 세션의 언어가 `EN`이었다. 로그인 대신 토큰을 주입해 검증한 탓이며
코드 문제가 아니다.

### 8-3. 검증 방법 메모

⚠️ PR #311은 **CodeRabbit이 rate limit으로 리뷰를 돌지 못했다.** 체크가 초록이지만
"지적 0건"이 아니라 "검토 안 됨"이다. CI 게이트와 자체 검증만으로 머지했다.
