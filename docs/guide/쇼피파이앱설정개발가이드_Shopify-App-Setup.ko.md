...
| Shopify Admin API 클라이언트 | ✅ (온디맨드) | `POST /api/v1/tenants/me/shopify/sync` — 저장된 토큰으로 `orders.json` 조회 → `orders_cache`·`customers` upsert. 스케줄 자동 동기화는 로드맵 |
...
| `embed.js` 로더 / iframe 임베드 | ⛔ | 스토어 주입 로더·iframe 아직 |
