...
| 기본값 | 탭 `알림`·`주문`·`채팅` 3단, 위치 `상단` — ⚠️ 이 문서는 2탭 기본으로 작성되었고, 같은 라운드 중 사용자 지시로 3탭 기본으로 바뀌었습니다(§변경 이력). 아래 본문의 "2탭" 서술은 그 지시 이전 시점 기록입니다. |
...
| # | 케이스 | 기대 |
|---|---|---|
| R-1 | **미설정 테넌트** | (3탭 전환 후) 상단 3탭 — 알림·주문·채팅, 알림 칩 4개·주문 칩 2개 |
...
## 9. 변경 (2026-08-18) — 기본 탭이 3단으로

사용자 지시: "탭 3단을 기본으로 (알림/채팅/주문)".

| 항목 | 이전 | 이후 |
|---|---|---|
| `WIDGET_TABS_DEFAULT` | `['notifications','chat']` | **`['notifications','orders','chat']`** |
| 표시 순서 | `알림 → 주문 → 채팅` | **변경 없음**(사용자 확인) — 목록형 탭 둘이 인접해 칩 이동이 자연스럽다 |
| 마이그레이션 | — | **불필요** |

**한 줄로 끝나는 이유**: `widget_tabs`를 NULL로 두는 설계(§2.1) 때문이다. 어떤 테넌트 행도
옛 기본값을 복사해 갖고 있지 않으므로, 상수 한 줄을 바꾸면 미설정 테넌트 전부가 다음
`session/ensure`에서 새 기본값을 따라온다. 기본 배열을 행에 써 넣었다면 여기서 백필이 필요했다.

**부수 효과**: 미설정 테넌트의 위젯이 **실제로 바뀐다**(2탭 → 3탭, 알림 탭 칩이 6개 → 2개로
분리). 이는 지시된 변경이며, 그동안 "기본값 = 무변화"를 지켜온 것과는 성격이 다르다.

**테스트**: 기본값을 단언하던 스펙 3곳이 리터럴을 쓰고 있어 `WIDGET_TABS_DEFAULT` 참조로
바꿨다 — "미설정은 기본값을 따른다"는 명제는 기본값이 바뀌어도 참이어야 하고, 리터럴은
그때마다 고쳐야 하는 스냅샷일 뿐이다.
