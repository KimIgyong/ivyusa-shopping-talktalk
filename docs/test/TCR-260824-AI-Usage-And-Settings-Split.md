...
### 1-3. 사용량 보존 (`retention.service.spec.ts`, 6건 — #369)
| ID | 케이스 | 기대 |
|---|---|---|
| P-1 | 기본 창 | **400일**, 대화 창과 무관 |
| P-2 | 기준 컬럼 | `stat_date` — 누적으로 움직이는 `updated_at`이 아님 |
| P-3 | 대화 창을 30일로 줄임 | 사용량 창은 **따라가지 않음** |
| P-4 | 명시적 창 60일 | 그대로 적용 |
| P-5 | 파싱 불가 값 | 기본값 폴백 — **0을 액면대로 읽으면 테이블 전체가 날아감** |
| P-6 | 결과·감사 로그 | `aiUsage`·`aiUsageRetentionDays` 기록 |

### 배포 검증 (#369)
| ID | 확인 | 결과 |
|---|---|---|
| S-15 | 부팅 | `Retention purge scheduled every 24h` |
| S-16 | 배포본에 새 창 | `AI_USAGE_RETENTION_DAYS', 400` |
| S-17 | ⚠️ 스테이징 env | 값 없음 → **기본값 400일로 동작** |

## 4. 잔여
- **R1.** §3 전체(콘솔 실화면).
- **R2.** 계측은 **2026-08-24부터** 데이터가 존재합니다. 그 이전 사용량은 없습니다(소급 불가).
- **R4.** 보존 정책은 배포됐으나 **실제 폐기는 400일 뒤**에야 처음 일어납니다. 그때까지
  동작을 확인할 방법은 창을 임시로 줄여보는 것뿐입니다(단위 테스트 P-1~P-6으로 대체).
