...
## 10b. 결정 기록 (2026-08-08, 사용자 확정 — §9/§11.3 전 항목)

| # | 결정 |
|---|---|
| 1 | 이슈↔대화 **1:1 승격**, inquiry 스텁은 흡수(신규 기능은 issues로) |
| 2 | 티켓 생성 = **에스컬레이션 + 특정 유형만**(단순 자동해소 문답은 미생성 — 카드 폭주 방지) |
| 3 | 반려 = 정책불가/오분류/스팸 **3사유 코드** + 사유별 고객 문구 템플릿 |
| 4 | 배정 축 = **기존 라벨**(consult/accounting/operations), 부서 신설 없음 |
| 5 | SLA = 기존 handoff 영업시간 재사용 + 유형별 목표시간 2단계(일반/긴급) |
| 6 | 상태알림 = 위젯 Inquiries+웹푸시, email은 기존 off-hours 경로 재사용, SMS 보류 |
| 7 | 실시간 전송 = 폴링 유지(P5 재검토) |
| 8 | 제휴/B2B = 이슈 유형 라벨 구분만(워크플로우 분리 없음) |
| 9 | 지식 폐루프 = 자동제안 + **사람 승인까지만**(자동승인 금지) |
| 10 | 권한 = 상태 전이: 담당자+manager 이상 / 반려·재배정: manager 이상 |
| 11 | 모드 B 심도 = **L1(생성-only) 출시 → L2**(상태회신) 순차 |
| 12 | 재-에스컬레이션 = 외부 티켓 open이면 append, closed면 신규 |
| 13 | 커넥터 순서 = Gorgias 검증 후 수요 기반 결정 |
| 14 | (해소) A모드 파일럿 = amoebaorder(Cafe24) — 커머스 연동 전제조건 2026-08-08 충족 |

→ P1 PLN: `docs/plan/PLN-260808-Issue-Workflow-P1.md`
