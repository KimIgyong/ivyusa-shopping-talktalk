...
### 실행 기록 (2026-08-13, 스테이징)

**E5 실측 → 결함 발견.** 저장된 항목과 완전히 같은 질문·언어·출처로 물어도 **재생되지 않았다**
(5건 전부 `hit_count=0`). 원인은 저장은 `document`, 조회는 `query`로 임베딩해 **같은 문장의 내적이
0.61**밖에 안 나오는 것 — 임계값 0.92를 구조적으로 넘을 수 없다.
상세·수정 방향: `docs/bug-fix/FIX-260813-AnswerReuse-Never-Replays.md`.
⚠️ **E6·E7은 E5 수정 전까지 검증 불가**(재생이 전제인 시나리오).

E1~E4는 위젯 화면 조작이 필요해 미실시.
