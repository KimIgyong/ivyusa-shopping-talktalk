...
외부 메신저 연동 PR-M1~M3 구현 완료 보고 — **스테이징 배포 완료 (2026-08-10)**.

- 근거: `REQ-260810` · `PLN-260810` (Rev.3) · `TCR-260810`
- 범위: 코어 프레임 + 텔레그램·바이버(직접) + 아메바톡 허브 + btbz KSR 릴레이
- 미착수(후속): PR-M4 지메일 IMAP + 콘솔 채널 뱃지 · PR-M5 설정 카드 UI
...
→ 스테이징 **코드 배포 전 선적용 완료**(`ivy_mysql_staging`, 4테이블·스키마 검증). M2/M3은 스키마 변경 없음.
프로덕션은 호스트 미구축 상태라 미적용.
...
- 단위 **886 passed / 84 suites**(신규 68) · typecheck 9/9 · 실제 부팅 3회 확인
...
| 구현 | **PR-M4** 지메일 IMAP 어댑터 + 콘솔 대화 리스트 채널 뱃지·필터·미확인 발신 표기 | 없음(착수 가능) |
| 구현 | **PR-M5** 설정 "외부 메신저 연동"·"커뮤니케이션 연동" 카드 UI | 없음(착수 가능) |
